...
src/
  ...
  services/
    data-fetcher.ts         ← Unified multi-source fetcher (Upstox → NSE → Yahoo)
    upstox-api.ts           ← Upstox REST API client
    nse-scraper.ts          ← NSE India direct scraper (cookie-based)
    yahoo-finance.ts        ← Yahoo Finance wrapper (fallback)
    indicators.ts           ← RSI, MACD, Bollinger, Stochastic, etc.
    patterns.ts             ← 10 chart pattern detectors
    analysis.ts             ← Weighted scoring engine (BUY/SELL/HOLD)
    probability.ts          ← Platt scaling calibration
    fundamentals.ts         ← P/E, Market Cap, ROE (₹ formatting)
    news.ts                 ← Google News India RSS
    earnings.ts             ← Quarterly earnings data
    dividends.ts            ← Dividend history (₹/share)
    options.ts              ← Options chain (NSE/Upstox/Yahoo)

  commands/
    predict.ts              ← Main prediction pipeline
    optimize.ts             ← Standalone backtesting optimizer
    learn.ts                ← Full self-improvement loop

  optimization/
    optimizer.ts            ← Random search parameter optimizer
    backtester.ts           ← Trade simulator with Indian tx costs (0.15%)
    calibrator.ts           ← Platt scaling (Brier score minimization)
    data-loader.ts          ← Historical data loader for backtesting
    evaluator.ts            ← Prediction accuracy metrics
    types.ts                ← Optimization interfaces

  portfolio/
    manager.ts              ← JSON-based portfolio CRUD

  reports/
    generator.ts            ← Markdown report generator (₹)

  utils/
    config-loader.ts        ← Load/save optimized_weights.json
    csv-writer.ts           ← CSV output writer
    json-exporter.ts        ← JSON output writer
    slack.ts                ← Slack webhook notifications

data/
  config/optimized_weights.json  ← Persisted optimization results
  feedback/predictions_*.json    ← Daily prediction history
